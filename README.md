...
The TMR0 peripheral is configured as an 8-bit timer to create a two millisecond periodically signal. This signal is used to control the partial steps rate accordingly to the specifications of the Switec stepper motor.
...
The positive edge of the TMR1 overflow is used to stop the waveform sequence applied to the Switec stepper motor when the number of partial steps are reached. The edge detector introduces two CLB clocks delay (four milliseconds). To avoid missing of steps, an CLB interrupt request (CLB_IRQ0) is generated when the step sequence is complete (on the edge of TMR1 overflow).
